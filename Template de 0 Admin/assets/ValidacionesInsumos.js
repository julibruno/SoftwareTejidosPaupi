...
function ejecutaAlertaOK() {   
    ...
    var ValorRepetido= obtenerValorParametro("Yaexiste");
    var Eliminado= obtenerValorParametro("Eliminado");
    
    if(ValorRepetido){
    escribirDatoRepetido(ValorRepetido);
    $('#alertaError').fadeIn();     
    setTimeout(function() {
        $("#alertaError").fadeOut();           
    },20000);
...
//Escribir dato repetido
function escribirDatoRepetido(VariableRepetido){
  var mensaje=  "Error! El insumo ingresado ya existe. Intente con otra Descripcion";
   document.getElementById('alertaError').innerHTML= mensaje;
   }

function escribirEliminado(Eliminado){
  ...
  function Validacion_Create(e){
    ...
    if(this.querySelector('[name=Descripcion]').value == '') { 
      ...
      if(this.querySelector('[name=Color]').value == '') { 
        console.log('El color esta vacía');
